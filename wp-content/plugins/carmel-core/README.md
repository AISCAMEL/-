...
- **並び替え**：在庫一覧を**新着順／価格が安い順／高い順／人気順**（`?sort=`・人気は `_carmel_views` 閲覧数）で並び替え
- **閲覧履歴（最近見た車）**：在庫詳細の閲覧で `_carmel_views` を加算し会員の `carmel_recent_vehicles` に記録。一覧・詳細に「最近見た車」ストリップを表示
...
3. **固定ページを作成**し各ショートコードを設置：
   ...
   - `/mypage` → `[carmel_mypage]`＋`[carmel_upload]`（任意で `[carmel_my_documents]`／`[carmel_customer_guide]`／`[carmel_my_stores]`）
