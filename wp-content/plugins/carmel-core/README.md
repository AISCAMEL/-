...
- **CSV一括取込**：`[carmel_store_inventory]` の「在庫をCSVで一括取込」から、`maker,model,grade,year,mileage,color,vin,plate_no,price,cost,vehicle_status,published`（本部は `store_id` 列で店舗指定可）をまとめて登録。UTF-8/Shift_JIS対応・最大500行・「取込時に全公開」オプションつき。**記入例つきテンプレCSVのダウンロード**も可
