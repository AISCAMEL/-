...
| 💳 **オートローン** | 車両価格・頭金・回数・実質年率から**元利均等の月々支払い**を試算（JS即時）→ 支払シミュレーションを見積書として発行。案件選択時、申込からの希望条件（`extra_loan_price`/`down`/`months`）を**自動プリフィル** |
...
- **並び替え＋ページネーション**：新着順／価格が安い順／高い順／人気順（`?sort=`）＋**ページ送り**（`?paged=`・1ページ `carmel_inventory_per_page` 既定12／走査上限 `carmel_inventory_scan_cap` 既定300）
- **閲覧履歴（最近見た車）**：在庫詳細の閲覧で `_carmel_views` を加算し会員の `carmel_recent_vehicles` に記録。一覧・詳細に「最近見た車」ストリップ。単体 **`[carmel_recent_vehicles]`**（マイページ用）
- **ローン月々の概算**：在庫**カード**に「月々¥〜」（頭金0・既定条件）、在庫**詳細**に頭金・回数・年率を調整できるライブ計算ウィジェット（`carmel_finance_defaults`）
- **こだわり条件絞り込み**：ボディタイプ（ACF `body_type`）・年式（◯年以降）・走行距離（◯km以下）で在庫を絞り込み
- **簡易事前審査** `[carmel_prescreen]`：希望価格・頭金・年収・雇用形態・他社借入から**借入可能額/月々/判定の目安**をその場で提示（参考値・クライアント計算）＋審査申込CTA
- **お気に入り**：ログインユーザーは各車両を♥でお気に入り登録（`carmel_favorites` user_meta）。`?fav=1` でお気に入りのみ表示、タブで切替。**お気に入りをまとめて比較**（サーバー側のお気に入りから比較URLを生成）
- **比較**：カードの「比較に追加」で最大4台を選択（localStorage）。比較バーから side-by-side の**比較表**（画像・価格・**月々目安**・スペック・取扱店）を表示
- **新着/値下げバッジ**：公開から `carmel_inventory_new_days`（既定7日）以内は **NEW**、価格が下がった車両は **値下げ**バッジ＋旧価格の取り消し線（`price_prev` は価格更新を `update_post_metadata` で検知し記録）。**値下げ時はお気に入り登録会員へ通知**（`price_drop`・プロライン→メール）
- **SOLD（売約済）表示**：既定は販売中/商談中のみ。「売約済も表示する」で `売約済`/`納車済` を **SOLDオーバーレイ**付きで表示（CTAは無効化・`?show_sold=1`）
- **近日入荷＋入荷通知**：在庫ステータス `入庫予定` の公開車両を一覧上部に「🆕 近日入荷」表示。**「入荷したら通知」登録**（`_arrival_waitlist`）→ 販売中へ変わると登録者へ `inventory_arrived` 通知
- **商談の次アクション期日**：`/store` の案件に「次アクション＋期日」を設定（ACF `next_action`/`next_action_due`）。期日到来で担当店へ `deal_action_due` 通知（`Carmel_Deal_Followup` 日次）
- **反響→商談→成約ファネル**：`[carmel_hq_reports]` に反響受付→商談化→成約の件数＋転換率を表示（スコープ＝本部全店/加盟店自店）
- **値下げ通知の対象拡張**：お気に入り登録者に加え、**取扱店フォロワー・保存検索が一致するユーザー**にも `price_drop` を1人1通で通知

## 商談タイムライン（実装済み）

`Carmel_Deal_Timeline`。ショートコード **`[carmel_deal_timeline]`**（`?deal=ID`・固定ページ `/deal` 想定）。案件の**ステータス履歴＋対応メモ＋起票**を時系列で表示。閲覧は本部／担当加盟店／本人（顧客）、**メモ追加は本部・担当店のみ**（メモ追加で停滞アラートもリセット）。`/store` の案件行に「履歴」リンク。

## 商談フォロー漏れアラート（実装済み）

`Carmel_Deal_Followup`。一定日数（`carmel_deal_stale_days`・既定7日）**ステータスの動きがない進行中案件**を、日次cronで**担当店＋本部へ通知**（`deal_stale`）。ステータスが動くと活動日時を更新し再通知フラグをリセット（1回のみ通知）。決着済み・後工程（成約後/クローズ/否決）は対象外。
- **事前審査/在庫詳細→申込フォーム引き継ぎ**：`?vehicle=&price=&down=&months=` を申込フォームが受け取り「ご希望条件」を表示＋`extra_` メタとして案件に保存（事前審査・月々ウィジェットのCTAが自動付与）
...
- **店舗詳細（`?store=ID`）**：店舗名・住所・電話・営業時間・紹介文・**地図**＋**実績**（公開在庫/成約/取扱案件）＋**スタッフ紹介**（自店の owner/staff・`carmel_store_show_staff` で制御）＋**その店の公開在庫カード**＋「この店舗の在庫をもっと見る」（`/inventory?store_id=ID`）＋**レビュー**（★評価・承認制／ハニーポット＋nonce）＋**この店舗への問い合わせフォーム**（送信→`carmel_support`（`store_inquiry`）として反響起票→担当店＋本部へ通知・/store の反響一覧に表示・商談化可）
...
- **お気に入り店舗**（`Carmel_Store_Follow`）：店舗ページでフォロー（`carmel_followed_stores`）。フォロー店舗が在庫を公開すると**新着通知**（`store_new_stock`・プロライン→メール）。マイページに **`[carmel_my_stores]`**（フォロー中の店舗一覧・解除）
- **曜日別営業時間・定休日**：店舗ページに曜日別表（ACF `store_open`/`store_close`/`store_closed_days`）。SEOに `openingHoursSpecification` を出力
- **加盟店ディレクトリ地図**：一覧に**複数マーカー地図**（店舗住所をジオコーディングし `store_lat`/`store_lng` にキャッシュ・在庫地図と共通／`CARMEL_MAPS_API_KEY` 設定時）
...
3. **固定ページを作成**し各ショートコードを設置：
   ...
   - `/mypage` → `[carmel_mypage]`＋`[carmel_upload]`（任意で `[carmel_my_documents]`／`[carmel_customer_guide]`／`[carmel_my_stores]`／`[carmel_recent_vehicles]`）
   ...
   - `/deal` → `[carmel_deal_timeline]`（商談タイムライン・`?deal=ID`）
