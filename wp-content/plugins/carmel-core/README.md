...
**デザインのカスタマイズ**（ショートコード属性）：
```
[carmel_login
  logo="https://carmelonline.jp/logo.png"   ← ロゴ画像URL（無ければ文字ロゴ）
  wordmark="CARMEL"                          ← 文字ロゴ
  tagline="クルマのことは、カーメルにおまかせ。"  ← キャッチコピー
  brand="#1a1a2e" accent="#2e86de"]          ← メイン／アクセントカラー
```
...
3. **固定ページを作成**し各ショートコードを設置：
   ...
   - `/store` → `[carmel_store]`
   - `/hq` → `[carmel_hq_screening]` `[carmel_hq_board]` `[carmel_hq_contracts]` `[carmel_hq_reports]` `[carmel_hq_stores]`
