...
- **CSV一括取込（VIN upsert）**：`[carmel_store_inventory]` の「在庫をCSVで一括取込」から、`maker,model,grade,year,mileage,color,vin,plate_no,price,cost,vehicle_status,published`（本部は `store_id` 列で店舗指定可）をまとめて登録。UTF-8/Shift_JIS対応・最大500行・「取込時に全公開」オプション・**記入例つきテンプレCSVのDL**。**車台番号(VIN)が一致する既存在庫は更新（重複作成しないupsert）**＝再取込で在庫を最新化（加盟店は自店内、本部は全体で照合）。結果は「新規N件・更新M件」で表示
...
- **保存検索・新着アラート**：現在の絞り込み条件を保存（`carmel_saved_searches` user_meta・最大10件）。日次cronで**保存条件に合う新着公開在庫を検出し本人へ通知**（`inventory_new_arrival`・LINE→メール）。保存条件はワンクリックで再検索・削除可
...
- **組み込み掲示板 `[carmel_community]`**（`/community`・bbPress非依存・CARMEL内で完結）
  ...
  - **いいね**：トピック・返信に👍（`_carmel_likes`）。**ベストアンサー**：トピック投稿者/本部が返信を1件ベスト指定（先頭固定＋一覧に「✔解決済」）
...
3. **固定ページを作成**し各ショートコードを設置：
   ...
   - `/mypage` → `[carmel_mypage]`＋`[carmel_upload]`（任意で `[carmel_my_documents]`）
   ...
   - `/hq` → `[carmel_hq_screening]` `[carmel_hq_board]` `[carmel_hq_contracts]` `[carmel_hq_reports]` `[carmel_hq_stores]` `[carmel_hq_commissions]`
