...
- **WPネイティブ フォーム**：お問い合わせ / 資料請求 / 無料体験 / ミーティング予約。送信は REST、データは「お問い合わせ」CPT に保存。プラグイン不要。
- **導線ごとに最適化した自動返信＋シーケンス**（申込直後に種別別の自動返信、以降は wp-cron で自動配信。配信停止リンク付き）：
  - **資料請求** → 資料活用フォロー（1/3/7/14/30日）
  - **見積もり・発注** → 検討促進（1/3/7/14/30日。発注時の自動返信に見積明細を同封）
  - **30日お試し** → 体験ナーチャ＋終了前リマインド（1/3/7/14/25/30日）
  - **ミーティング予約** → 予約日時を基準に**前日・直前リマインダー＋翌日フォロー**
  - **お問い合わせ** → 長期ナーチャ（〜365日）
  - 文面は `apprex_step_mails` / `apprex_meeting_reminders` フィルタで編集可
