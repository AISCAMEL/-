...
<p>4.（中途解約・違約金）乙が最低利用期間（契約開始日から起算して１２ヶ月間をいう。以下同じ。）の満了前に、退会その他事由の如何を問わず本サービス利用契約を終了する場合（第１０条に基づき甲が解除した場合を含む。）、乙は甲に対し、違約金として次の各号の合計額を、解約日の属する月の翌月末日までに一括して支払うものとする。<br>
(1)キャンペーン又は割引により無料若しくは減額とした初期設定費がある場合は、その割引相当額（本来の初期設定費の金額から実際に支払った金額を控除した額）<br>
(2)解約日の属する月の翌月から最低利用期間の満了月までの残存月数に、月額利用料を乗じた額<br>
なお、本項の違約金は損害賠償額の予定とし、既に支払済みの初期費用・制作費・月額利用料を返金しない旨（本条第１項）の定めを妨げるものではない。アプリ登録費その他の実費は別途乙の負担とする。</p>
...
/**
 * 保存済みテンプレに「中途解約・違約金」項を一度だけ追加（未挿入の場合のみ）。
 */
add_action( 'admin_init', function () {
	if ( get_option( 'apprex_contract_doc_penalty_v1' ) ) {
		return;
	}
	$tpl = (string) get_option( 'apprex_contract_template', '' );
	if ( '' !== $tpl && false === mb_strpos( $tpl, '中途解約' ) ) {
		$anchor = '3.契約開始日は {{start}}（申込日の翌月の一日）とし、申込日から契約開始日までは無料でご利用できる。</p>';
		$clause = $anchor . "\n" .
			'<p>4.（中途解約・違約金）乙が最低利用期間（契約開始日から起算して１２ヶ月間をいう。以下同じ。）の満了前に、退会その他事由の如何を問わず本サービス利用契約を終了する場合（第１０条に基づき甲が解除した場合を含む。）、乙は甲に対し、違約金として次の各号の合計額を、解約日の属する月の翌月末日までに一括して支払うものとする。<br>' . "\n" .
			'(1)キャンペーン又は割引により無料若しくは減額とした初期設定費がある場合は、その割引相当額（本来の初期設定費の金額から実際に支払った金額を控除した額）<br>' . "\n" .
			'(2)解約日の属する月の翌月から最低利用期間の満了月までの残存月数に、月額利用料を乗じた額<br>' . "\n" .
			'なお、本項の違約金は損害賠償額の予定とし、既に支払済みの初期費用・制作費・月額利用料を返金しない旨（本条第１項）の定めを妨げるものではない。アプリ登録費その他の実費は別途乙の負担とする。</p>';
		$new = str_replace( $anchor, $clause, $tpl );
		$new = str_replace( '第６条（料金、支払方法、契約期間）', '第６条（料金、支払方法、契約期間、中途解約）', $new );
		if ( $new !== $tpl ) {
			update_option( 'apprex_contract_template', $new );
		}
	}
