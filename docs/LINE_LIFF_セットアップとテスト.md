...
### 3-2. 会話型ヒアリング（反響起票）
- [ ] クイックリプライ「チャットで相談」→ 氏名→電話→内容 の順に質問される
- [ ] 完了で「受け付けました」。WP管理画面 `carmel_support` に **LINE反響：氏名**（`support_type=line_inquiry`・`line_user_id`）が作成される
- [ ] 本部に `line_lead` 通知が届く（LINE WORKS / メール）
- [ ] 途中で「キャンセル」→ 中止できる
