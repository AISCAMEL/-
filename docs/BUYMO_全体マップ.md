...
| URL | 役割 | 認証 |
|---|---|---|
| `buymo.html` | 買取トップLP（査定シミュ・比較表・追従CTA・査定フォーム・会員誘導／Organization・WebSite・AutoDealer・FAQ 構造化データ） | 公開 |
| `buymo-contact.html` | お問い合わせ／無料査定フォーム（`?genre=`/`?pref=`/`?est=` を文脈バナー＋自動入力で受取） | 公開 |
| `buymo-thanks.html` | 送信完了（サンクス）ページ。来訪ジャンル表示・`conversion` 計測・次の流れ | 公開(noindex) |
| `buymo-partner.html` | パートナー（加盟店）募集＋応募フォーム | 公開 |
| `genre/` | 買取ジャンルハブ（5大分類＝状態/人気車種/タイプ区分/旧車希少/パーツ用品） | 公開 |
| `genre/<slug>/` | **ジャンル専用LP×25**（固有title/desc/H1・買取目安・ジャンル別FAQ・査定シミュ・対応エリア・AutoDealer/Breadcrumb/FAQ 構造化データ）。全`live` | 公開 |
| `genre/<slug>/<県>/` | **ジャンル×エリア 掛け合わせLP×48**（商用上位6ジャンル×主要8都市・地域＋ジャンル合成コピー・4階層パンくず） | 公開 |
| `area/` ＋ `area/<県>/` | 都道府県別SEO LP（ハブ＋47県・人気ジャンルへ相互リンク） | 公開 |
...
| ファイル | 役割 | 設定 |
|---|---|---|
| `buymo.js` | フォーム検証・送信→サンクス遷移・`?genre/pref/est`受取・スムーズスクロール・FAQ・カルーセル・追従CTA 等 | `ENDPOINT` |
| `simulator-buy.js` | 査定シミュレーター（再利用可・`[data-buymo-sim]`自動初期化・車種/状態プリセット・CTAに`genre/pref/est`付与） | 係数 `CLASS_BASE` 等 |
| `genres.js` | ジャンル一元管理（**2階層 GROUPS/LIST**・全`live`）→ フッター/LP/ハブ/掛け合わせに自動反映 | `GROUPS` |
...
## 2.5 生成・QA（`site/tools/`）
| ファイル | 役割 |
|---|---|
| `_layout.js` | 生成ページ共通のヘッダー／フッター（フッターに analytics・genres・buymo・simulator-buy を読込） |
| `_cross.js` | ジャンル×エリア掛け合わせの対象（6ジャンル×8都市）を一元定義。生成とsitemap収録が共有 |
| `gen-genre.js` | `genres.js`→ ジャンルハブ＋25ジャンルLP＋48掛け合わせLP を静的生成 |
| `gen-area.js` | 47都道府県LP＋ハブ＋`sitemap.xml`（ジャンル/掛け合わせURLも収録）を生成 |
| `check-links.js` | 内部リンク／アセットの健全性チェック |
| `seo-check.js` | 全HTMLのJSON-LD検証＋title重複/canonical/description/H1/OGP＋データ整合（genres・cross・sitemap） |
| `launch-check.js` | 公開準備の実状態を判定（ENDPOINT/GA4/SITE_URL/画像/ページ数）。`node tools/launch-check.js` |

> 生成コマンド：`cd site && node tools/gen-genre.js && node tools/gen-area.js`。
> QA：`node tools/check-links.js && node tools/seo-check.js`。両者は **GitHub Actions（`.github/workflows/qa.yml`）でpush毎に自動実行**。
...
- [ ] 架空の声・実績・買取目安を実データへ（現状「※イメージ」明記）
- [x] ジャンルLP（25）・掛け合わせLP（48）を**内製生成**し全`live`／`sitemap.xml`収録（計125URL）。※外部LPtools併用も `genres.js` 同期で可
- [ ] ジャンルのヒーロー画像配置（`assets/img/genre/<slug>.jpg` を置くと自動表示）
