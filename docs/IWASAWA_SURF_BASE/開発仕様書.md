...
| Step | 内容 | 対象 | 状況 |
|------|------|------|------|
| 0 | リポジトリ初期化(Next.js+Supabase)・デザイントークン | - | ✅ |
| 1 | 認証＋members(role×plan) | C-04/05/06 | ✅ |
| 2 | 権限ゲート(共通化・RLS) | 横断 | ✅ |
| 3 | ホーム＋コミュニティ・フィード | 5-A / C-01 | ✅ |
| 4 | 投稿の中身(詳細/作成/いいね/コメント) | C-02/07 | ✅ |
| 5 | マイページ(基本)＋自分のスキル | C-08 / SK-05 | 🔄 |
...
実装の現状（2026-06）：Step0〜9 をコード実装済み（`web/`）。Supabase 接続後に実機検証する段階。
残り：Step5 の自分の投稿一覧(C-09)・プロフィール編集、PWA化、Step10（決済・広告・有料会員・ネイティブ）。
