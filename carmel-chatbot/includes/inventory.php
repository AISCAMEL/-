...
/** お客様が「在庫を見たい／探している」を明確に示したかだけを判定。
 * ※ 単に「予算が」「燃費が」など話題に触れただけでは在庫を出さない。
 */
function carmel_cb_is_car_intent( $text ) {
	$text = (string) $text;
	if ( $text === '' ) { return false; }
	return (bool) preg_match(
		'/(在庫(を|は|が)?(見|教|ある|ない|確認|チェック)|見せて|見たい|見れます|見せてください|(車|クルマ|中古車)(を|が)?(探|さがし|見|見せ|教|紹介|オススメ|おすすめ|候補|提案)|どんな(車|クルマ|中古車|在庫)|一覧|(在庫|車)を?出|(車|クルマ|中古車).*ありますか|(何|なに)(か|が)?(あり|ある)ますか)/u',
		$text
	);
}
