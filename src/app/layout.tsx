import type { Metadata } from "next";
import { Zen_Kaku_Gothic_New } from "next/font/google";
import "./globals.css";
import { Header } from "@/components/layout/Header";
import { Footer } from "@/components/layout/Footer";
import { site } from "@/content/site";

const sansJp = Zen_Kaku_Gothic_New({
  subsets: ["latin"],
  weight: ["400", "500", "700"],
  display: "swap",
  variable: "--font-sans",
});

export const metadata: Metadata = {
  metadataBase: new URL(site.url),
  title: {
    default: `${site.name}｜自動車（販売・買取・オンライン販売・セキュリティ・レッカー）／IT・WEB・FC事業`,
    template: `%s｜${site.name}`,
  },
  description: site.description,
  keywords: [
    "カーメル",
    "BUYMO",
    "CARSHICO",
    "天護",
    "TENGO",
    "APPREX",
    "WEB crews",
    "AIオペレーター24",
    "国産車 販売",
    "車 買取",
    "農機具 買取",
    "アルミ 買取",
    "新車 オンライン 購入",
    "GPS 遠隔停止",
    "車両セキュリティ",
    "レッカー カーレスキュー 福島",
    "ノーコード アプリ開発",
    "サブスク ホームページ制作",
    "いわき市 自動車",
  ],
  openGraph: {
    type: "website",
    locale: "ja_JP",
    siteName: site.name,
    title: `${site.name}｜自動車（販売・買取・オンライン販売・セキュリティ・レッカー）／IT・WEB・FC事業`,
    description: site.description,
    url: site.url,
  },
  twitter: {
    card: "summary_large_image",
    title: `${site.name}`,
    description: site.description,
  },
  robots: { index: true, follow: true },
  alternates: { canonical: "/" },
};

export default function RootLayout({
  children,
}: {
  children: React.ReactNode;
}) {
  return (
    <html lang="ja" className={sansJp.variable}>
      <body>
        <a
          href="#main"
          className="sr-only focus:not-sr-only focus:fixed focus:left-4 focus:top-4 focus:z-[60] focus:rounded-md focus:bg-brand-600 focus:px-4 focus:py-2 focus:text-white"
        >
          本文へスキップ
        </a>
        <Header />
        <main id="main">{children}</main>
        <Footer />
      </body>
    </html>
  );
}
