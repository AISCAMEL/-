...
function ais_chat_options() {
	$defaults = array(
		...
		'greeting'     => 'こんにちは！合同会社アイズのAIアシスタントです。クルマの販売・買取・オンライン購入・セキュリティ、アプリ・Web制作、FCのことなど、お気軽にご質問ください。',
		'system_extra' => '',
		'api_key'      => '',
	);
...
function ais_chat_system_prompt() {
	$site = ais_site();
	$lines = array();
	$lines[] = 'あなたは「' . $site['name'] . '（' . $site['name_en'] . '）」のコーポレートサイトに常駐する、丁寧で親しみやすい日本語のAIアシスタントです。';
	$lines[] = '目的：訪問者の質問に、当社の事業内容にもとづいて正確に答え、最適な相談導線（お問い合わせフォーム）に案内すること。';
	$lines[] = '';
	$lines[] = '# 会社概要';
	...
	$lines[] = '';
	$lines[] = '# 回答ルール';
	$lines[] = '・日本語で、簡潔（目安2〜4文）かつ丁寧に回答する。';
	$lines[] = '・上記の事実のみにもとづく。価格・在庫・納期・個別条件など不確実な事項は断定せず、「お問い合わせフォームからご相談ください」と案内する。';
	$lines[] = '・電話番号は案内しない（窓口はメールまたはこのチャット）。';
	$lines[] = '・当社の事業と無関係な話題には、丁寧にお答えできない旨を伝え、事業に関する相談を促す。';
	$lines[] = '・具体的な相談・見積りはお問い合わせフォーム（/contact）を案内する。';
