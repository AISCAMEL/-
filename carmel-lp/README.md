...
| **AIの回答ナレッジ（FAQ）** | **`data/knowledge.csv`** を編集（下記参照） |
| **取扱車種（提案の元データ）** | **`data/vehicles.csv`** を編集（下記参照） |
...
ナレッジが増えても毎回すべてをAIに渡すのではなく、**お客様の質問に関連する数件だけ**を
自動で選んでAIに渡します（精度向上＋トークン削減）。仕組みは日本語向けの簡易スコアリング
（文字バイグラム一致＋車種の意図ブースト）で、外部サービス不要・`lib/carmel-bot.js` 内で完結します。

### 取扱車種（`data/vehicles.csv`）と車種提案

「家族で乗れる車」「燃費のいい軽」「アウトドア向けSUV」などの質問に対し、AIが
**`data/vehicles.csv`** から合いそうな車種を提案します。

- 列は `name,type,seats,price_range,features`。Excel/スプレッドシートで編集 → CSV(UTF-8)で上書き。
- 価格は**目安**として渡し、「最終的なお支払い・在庫は審査/面談/在庫状況による」とAIが必ず添えます。
- 用途キーワードの対応（家族→ミニバン / 軽・維持費→軽自動車 / 燃費→ハイブリッド / アウトドア→SUV）は
  `lib/carmel-bot.js` の `CAR_INTENTS` で調整できます。
- 別パスのCSVを使う場合は環境変数 `VEHICLES_CSV` で指定可能。
...
チャットの「📅 来店・査定を予約」から、種別・希望日・時間帯・連絡先を送信できます。

- **営業時間に関係なくいつでも受付**（AI常時・担当者は時間内、という方針と整合）。
- 送信内容は **Slackへ通知**（設定時）＋ **`data/bookings/` に控えを保存**。
  Slack未設定でも控えは残り、受付確認を表示します。
- 控えは `.gitignore` 済み（個人情報を含むため）。保存先は `BOOKINGS_DIR` で変更可。
- エンドポイント: `POST /api/handoff/reserve`。
