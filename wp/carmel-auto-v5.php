...
if (!defined('CARMEL_AUTO_OPTION')) {
    define('CARMEL_AUTO_OPTION', 'carmel_auto_settings');
}
if (!defined('CARMEL_AUTO_HOOK')) {
    define('CARMEL_AUTO_HOOK', 'carmel_auto_generate_cron');
}
if (!defined('CARMEL_IMG_DEFAULT_MODEL')) {
    define('CARMEL_IMG_DEFAULT_MODEL', 'google/gemini-2.5-flash-image-preview');
}
// 本文セクション画像を「裏側で1枚ずつ」生成するためのフック
if (!defined('CARMEL_AUTO_IMGHOOK')) {
    define('CARMEL_AUTO_IMGHOOK', 'carmel_auto_img_worker');
}
// 本文セクション画像 ACF フィールドキー（編集ページのHTMLから確認した値）
if (!defined('CARMEL_F_SEC1_IMG')) { define('CARMEL_F_SEC1_IMG', 'field_69ffb5a4d372b'); } // section_1_image
if (!defined('CARMEL_F_SEC2_IMG')) { define('CARMEL_F_SEC2_IMG', 'field_69ffb5e5d372e'); } // section_2_image
if (!defined('CARMEL_F_SEC3_IMG')) { define('CARMEL_F_SEC3_IMG', 'field_69ffb8a0c02c3'); } // section_3_image
...
function carmel_auto_get_settings() {
    $defaults = array(
        ...
        'fix_cta'      => 1,
        'contact_url'  => '',
        'image_model'  => CARMEL_IMG_DEFAULT_MODEL,
    ...
    $saved = get_option(CARMEL_AUTO_OPTION, array());
    if (!is_array($saved)) $saved = array();
    return wp_parse_args($saved, $defaults);
}

function carmel_auto_save_settings($settings) {
    update_option(CARMEL_AUTO_OPTION, $settings);
}

function carmel_auto_parse_queue($text) {
    $rows = array();
    $lines = preg_split('/\r\n|\r|\n/', (string)$text);
...
function carmel_auto_queue_to_text($queue) {
    $lines = array();
...
add_action(CARMEL_AUTO_HOOK, 'carmel_auto_run');
// 本文セクション画像を裏側で1枚ずつ処理するワーカー
add_action(CARMEL_AUTO_IMGHOOK, 'carmel_auto_process_one_image');

function carmel_auto_reschedule() {
    $s = carmel_auto_get_settings();
    $existing = wp_next_scheduled(CARMEL_AUTO_HOOK);
    if ($existing) {
        wp_unschedule_event($existing, CARMEL_AUTO_HOOK);
    }
    if (!empty($s['enabled'])) {
        $recur = in_array($s['frequency'], array('daily', 'carmel_weekly', 'carmel_twiceweekly'), true)
            ? $s['frequency'] : 'carmel_weekly';
        wp_schedule_event(time() + 60, $recur, CARMEL_AUTO_HOOK);
    }
}

add_action('init', function () {
    $s = carmel_auto_get_settings();
    if (!empty($s['enabled']) && !wp_next_scheduled(CARMEL_AUTO_HOOK)) {
        carmel_auto_reschedule();
    }
});

/* ===== 自動生成の本体 ===== */

function carmel_auto_run() {
    @set_time_limit(300);
    $s = carmel_auto_get_settings();

    if (!function_exists('carmel_generate_article_api')) {
        carmel_auto_log($s, '失敗: 既存プラグイン(carmel_generate_article_api)が見つかりません。CARMEL統合管理v5.7が有効か確認してください。');
        return false;
    }
    $queue = array_values((array)$s['queue']);
    if (empty($queue)) {
        carmel_auto_log($s, '失敗: テーマキューが空です');
        return false;
    }
    ...
    if ($item === null) {
        carmel_auto_log($s, '完了: テーマキューの全テーマを生成済みです（新しいテーマを追加してください）');
        $s['enabled'] = 0;
        carmel_auto_save_settings($s);
        $existing = wp_next_scheduled(CARMEL_AUTO_HOOK);
        if ($existing) {
            wp_unschedule_event($existing, CARMEL_AUTO_HOOK);
        }
    ...
    if ($title === '') {
        carmel_auto_log($s, "スキップ: テーマはタイトルもキーワードも空");
        return false;
    }
    ...
    if (empty($data['success'])) {
        ...
        carmel_auto_log($s, "失敗: {$title} / {$msg}");
        return false;
    }
    ...
    if (!empty($s['gmb_post']) && $post_id) {
        $gmb_msg = ' | Google: ' . carmel_auto_post_to_gmb($post_id, $item['account'] !== '' ? $item['account'] : 'main');
    }
    ...
    $remaining = carmel_auto_remaining_count($s);
    $status_label = !empty($s['publish']) ? '公開' : '下書き';
    carmel_auto_log($s, "成功: {$title}（{$status_label}） post#{$post_id}{$cta_msg}{$img_msg}{$gmb_msg} | 残り{$remaining}件");
    return $post_id;
}

function carmel_auto_item_key($row) {
...
function carmel_auto_remaining_count($s) {
    $queue = array_values((array)$s['queue']);
    $done  = isset($s['done']) && is_array($s['done']) ? $s['done'] : array();
    $remaining = 0;
    foreach ($queue as $row) {
...
function carmel_auto_log(&$s, $message) {
...
function carmel_img_mime_ext($mime) {
    $map = array('image/png' => 'png', 'image/jpeg' => 'jpg', 'image/jpg' => 'jpg', 'image/webp' => 'webp');
    return isset($map[$mime]) ? $map[$mime] : 'png';
}

function carmel_img_generate_openrouter($prompt, $model) {
    $key = carmel_img_api_key();
...
function carmel_img_sideload($post_id, $bytes, $mime, $base, $target_w = 0, $target_h = 0) {
    $ext = carmel_img_mime_ext($mime);
...
function carmel_img_attach_to_post($post_id) {
    $s = carmel_auto_get_settings();
    $model        = $s['image_model'] !== '' ? $s['image_model'] : CARMEL_IMG_DEFAULT_MODEL;
    ...
    $img = carmel_img_generate_openrouter($p1, $model);
    if (is_wp_error($img)) {
        $out[] = 'アイキャッチ失敗(' . $img->get_error_message() . ')';
    } else {
        $a = carmel_img_sideload($post_id, $img['bytes'], $img['mime'], 'eyecatch-' . $post_id, $ew, $eh);
        if (is_wp_error($a)) {
    ...
    $img2 = carmel_img_generate_openrouter($p2, $model);
    if (is_wp_error($img2)) {
        $out[] = 'バナー失敗(' . $img2->get_error_message() . ')';
    } else {
        $b = carmel_img_sideload($post_id, $img2['bytes'], $img2['mime'], 'banner-' . $post_id, $bw, $bh);
        if (is_wp_error($b)) {
...
function carmel_auto_get_acf_value($post_id, $field_key, $field_name) {
...
function carmel_auto_fix_cta($post_id, $s) {
    ...
    $targets = array(
        array(CARMEL_F_CTA1_URL, 'main_cta_url'),
        array(CARMEL_F_CTA2_URL, 'cta_button_url'),
    );
    $fixed = 0;
    foreach ($targets as $t) {
        $val = trim(carmel_auto_get_acf_value($post_id, $t[0], $t[1]));
        $bad = ($val === '')
        ...
        if ($bad) {
            carmel_auto_set_acf_value($post_id, $t[0], $t[1], $contact);
            $fixed++;
        }
    }
...
function carmel_auto_set_image_field($post_id, $field_key, $field_name, $att_id) {
    if (function_exists('update_field')) {
...
function carmel_auto_section_has_content($post_id, $n) {
    $candidates = array(
...
function carmel_auto_build_section_jobs($post_id, $s) {
    ...
    $fields = array(
        1 => array(CARMEL_F_SEC1_IMG, 'section_1_image'),
        2 => array(CARMEL_F_SEC2_IMG, 'section_2_image'),
        3 => array(CARMEL_F_SEC3_IMG, 'section_3_image'),
    );

    $jobs = array();
    foreach ($fields as $n => $f) {
        $existing = carmel_auto_get_acf_value($post_id, $f[0], $f[1]);
        if (trim($existing) !== '' && $existing !== '0') continue; // すでに画像あり→飛ばす
        $heading = carmel_auto_section_has_content($post_id, $n);
...
function carmel_auto_process_one_image($post_id) {
    ...
    if (is_array($job) && !empty($job['prompt'])) {
        $img = carmel_img_generate_openrouter($job['prompt'], $model);
        if (!is_wp_error($img)) {
            $att = carmel_img_sideload(
                $post_id, $img['bytes'], $img['mime'],
                $job['name'] . '-' . $post_id,
                (int)$job['w'], (int)$job['h']
            );
            if (!is_wp_error($att)) {
                carmel_auto_set_image_field($post_id, $job['key'], $job['name'], $att);
            }
        }
    }
...
function carmel_auto_post_to_gmb($post_id, $account = 'main') {
    // v5.7本体の関数が無ければ動かさない
...
add_action('admin_menu', 'carmel_auto_register_menu', 99);

function carmel_auto_register_menu() {
    add_menu_page(
        'CARMEL 自動生成',
        'CARMEL自動生成',
        'manage_options',
        'carmel-auto',
        'carmel_auto_settings_page',
        'dashicons-update',
        4
    );
}

add_action('admin_post_carmel_auto_save', function () {
    ...
    $s['image_model'] = $im !== '' ? $im : CARMEL_IMG_DEFAULT_MODEL;

    $bf = isset($_POST['banner_field']) ? sanitize_key($_POST['banner_field']) : '';
    $s['banner_field'] = $bf !== '' ? $bf : 'hero_image';

    $s['queue'] = carmel_auto_parse_queue(isset($_POST['queue']) ? wp_unslash($_POST['queue']) : '');
...
add_action('admin_post_carmel_auto_run_now', function () {
    if (!current_user_can('manage_options')) wp_die('権限がありません');
...
add_action('admin_post_carmel_auto_reset_done', function () {
    if (!current_user_can('manage_options')) wp_die('権限がありません');
...
add_action('admin_post_carmel_auto_process_image', function () {
    if (!current_user_can('manage_options')) wp_die('権限がありません');
    check_admin_referer('carmel_auto_process_image');

    $post_id = isset($_POST['post_id']) ? intval($_POST['post_id']) : 0;
    if ($post_id) {
        carmel_auto_process_one_image($post_id);
    }

    wp_safe_redirect(admin_url('admin.php?page=carmel-auto&imgran=1'));
    exit;
});

function carmel_auto_settings_page() {
    $s = carmel_auto_get_settings();
    ...
    <div class="carmel-wrap" style="max-width:1100px;margin:20px auto;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
        ...
        <div style="background:#fff;border:1px solid #e5e7eb;border-radius:14px;padding:18px;margin-bottom:16px">
            ...
            $remaining_count = carmel_auto_remaining_count($s);
            ?>
        ...
        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="background:#fff;border:1px solid #e5e7eb;border-radius:14px;padding:18px;margin-bottom:16px">
            ...
            <p><label>画像モデル（OpenRouter）：
                <input type="text" name="image_model" value="<?php echo esc_attr($s['image_model']); ?>" style="width:340px" placeholder="<?php echo esc_attr(CARMEL_IMG_DEFAULT_MODEL); ?>">
            </label><br>
            <span style="color:#666;font-size:12px">画像対応モデルを指定。例: <code><?php echo esc_html(CARMEL_IMG_DEFAULT_MODEL); ?></code></span></p>
            ...
            <textarea name="queue" style="width:100%;min-height:180px;font-family:monospace;font-size:13px;border:1px solid #d1d5db;border-radius:10px;padding:10px"><?php
                echo esc_textarea(carmel_auto_queue_to_text($s['queue']));
            ?></textarea>
        ...
        <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">
            ...
            ?>
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin:0">
                <input type="hidden" name="action" value="carmel_auto_process_image">
